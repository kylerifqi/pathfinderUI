...
(function () {
	...
	var QuickSettings = {
		...
		_createPanel: function (x, y, parent) {
			...
			this._panel.addEventListener('mousedown', function (e) { e.stopPropagation(); });
		...
		_createTitleBar: function (text) {
			...
			this._titleBar.addEventListener('mouseup', (ev) => {
				const diffX = Math.abs(ev.pageX - this._titleBarData.startX);
				const diffY = Math.abs(ev.pageY - this._titleBarData.startY);

				if (diffX < this._titleBarData.delta && diffY < this._titleBarData.delta) this._doubleClickTitle(ev);
			});
			this._titleBar.addEventListener('mousedown', (ev) => {
				this._titleBarData.startX = ev.pageX;
				this._titleBarData.startY = ev.pageY;
				this._startDrag(ev);
			});
